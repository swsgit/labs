...
size_t get_contacts(contact **c, FILE *fp) {
  char zip[64];
  size_t size = 0;
  for (size_t i = 0; i < MAX_CONTACTS; i++, size++) {
    ...
    if (c[i] == NULL)
      perror("Malloc failed");
    fgets(c[i]->name, MAX_BUF, fp);
    fgets(c[i]->address, MAX_BUF, fp);
    fgets(c[i]->locale, MAX_BUF, fp);
    fgets(zip, sizeof(zip), fp);
    c[i]->zip = atoi(zip);
...
void print_contacts(contact **c, size_t size, FILE *fp) {
  for (size_t i = 0; i < size; i++) {
    fprintf(fp, "%s%s%s%d\n", c[i]->name, c[i]->address, c[i]->locale,
            c[i]->zip);
  }
}
